...
class ApiHandle():
    ...
    def Environment(self,db: Session):
        try:
            current_time = datetime.datetime.now()
            formatted_time = current_time.strftime("%Y-%m-%d")
            token = os.getenv('TMD_TOKEN')
            if not token:
                raise ValueError("TMD_TOKEN is not set in the environment variables")
            headers = {
                'accept': 'application/json',
                'Authorization': f'Bearer {token}'
            }
            url_check = f"https://data.tmd.go.th/nwpapi/v1/forecast/location/hourly"
            r_check = requests.get(url_check, headers=headers)
            if r_check.status_code != 200:
                raise ValueError(f"Error fetching data from TMD API: {r_check.status_code} - {r_check.text}")
            data_check = json.loads(r_check.text)

            date = ''
            if 'hourly_data' in data_check and 'max' in data_check['hourly_data']:
                date = data_check['hourly_data']['max']

            location = db_handle.getLocation(db)
            cond = {
                "1": {"TH": "ท้องฟ้าแจ่มใส", "EN": "Clear"},
                "2": {"TH": "มีเมฆบางส่วน", "EN": "Partlycloudy"},
                "3": {"TH": "เมฆเป็นส่วนมาก", "EN": "Cloudy"},
                "4": {"TH": "มีเมฆมาก", "EN": "Overcast"},
                "5": {"TH": "ฝนตกเล็กน้อย", "EN": "Lightrain"},
                "6": {"TH": "ฝนปานกลาง", "EN": "Moderaterain"},
                "7": {"TH": "ฝนตกหนัก", "EN": "Heavyrain"},
                "8": {"TH": "ฝนฟ้าคะนอง", "EN": "Thunderstorm"},
                "9": {"TH": "อากาศหนาวจัด", "EN": "Verycold"},
                "10": {"TH": "อากาศหนาว", "EN": "Cold"},
                "11": {"TH": "อากาศเย็น", "EN": "Cool"},
                "12": {"TH": "อากาศร้อนจัด", "EN": "Veryhot"},
            }
            data_arr = []
            if location and date:
                for index, element in enumerate(location):
                    time.sleep(2)
                    url = f"https://data.tmd.go.th/nwpapi/v1/forecast/location/hourly/at?lat={
                        element['latitude']}&lon={element['longitude']}&fields=tc,rh,rain,cond,ws10m,wd10m&date={date}"
                    r = requests.get(url, headers=headers)
                    data = json.loads(r.text)
                    if 'WeatherForecasts' in data:
                        data['WeatherForecasts'][0]['plantCode'] = element['station_code']
                        if 'data' in data['WeatherForecasts'][0]['forecasts'][0]:
                            cond_code = str(data['WeatherForecasts']
                                            [0]['forecasts'][0]['data']['cond'])
                            data['WeatherForecasts'][0]['forecasts'][0]['data']['cond_th'] = cond.get(
                                cond_code, {}).get('TH')
                            data['WeatherForecasts'][0]['forecasts'][0]['data']['cond_en'] = cond.get(
                                cond_code, {}).get('EN')

                            data_arr.append(data['WeatherForecasts'][0])

            values = []
            if data_arr and 'data' in data['WeatherForecasts'][0]['forecasts'][0]:
                for item in data_arr:
                    ...
                    for forecast in item["forecasts"]:
                        ...
                        cond_th = forecast["data"].get("cond_th")
                        cond_en = forecast["data"].get("cond_en")
                        ws10m = forecast["data"]["ws10m"]
                        wd10m = forecast["data"]["wd10m"]
                        if cond_th and cond_en is not None:
                            values.append((station_code, forecast_time, rh,tc, rain, cond_th, cond_en, ws10m, wd10m))
            db_handle.insertEnvironment(values,db)
        except Exception as e:
